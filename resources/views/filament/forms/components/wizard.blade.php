@php
    ...
    $showIntermediateSave = (bool) (($extra['data-intermediate-save'] ?? false) ?: false);
...
>
                ...
                >
                    ...
                    >
                        <div class="fi-sc-wizard-header-step-icon-ctn">
                            @php $completedIcon = $step->getCompletedIcon(); @endphp
                            ...
                            @else
                                <span x-show="getStepIndex(step) <= {{ $loop->index }}" class="fi-sc-wizard-header-step-number">
                        ...
                        <div class="fi-sc-wizard-header-step-text">
                            @if (! $step->isLabelHidden())
                                <span class="fi-sc-wizard-header-step-label">{{ $step->getLabel() }}</span>
                            @endif

                            @if (filled($description = $step->getDescription()))
                                <span class="fi-sc-wizard-header-step-description">{{ $description }}</span>
                            @endif
                        </div>
                    </button>
    ...
    <div x-cloak class="fi-sc-wizard-footer">
        ...
        <div
            ...
            "
        >
            @if ($showIntermediateSave && filled($submitAction) && filled($alpineSubmitHandler))
                <span x-cloak x-show="! isLastStep()" style="display:inline-flex !important; flex:0 0 auto !important;">
                    <x-filament::button
                        type="button"
                        color="gray"
                        x-on:click.prevent="{!! $alpineSubmitHandler !!}"
            ...
            <span
                x-cloak
                x-show="! isLastStep()"
                style="display:inline-flex !important; flex:0 0 auto !important;"
            ...
            <span x-show="isLastStep()" style="display:inline-flex !important; flex:0 0 auto !important;">
                {{ $getSubmitAction() }}
            </span>
        </div>
    </div>
